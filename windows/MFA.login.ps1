# =========================================================================
# MFA_Login.ps1 - Autenticacion MFA con TOTP
...
function Write-MFALog {
    ...
    try {
        $carpeta = "C:\Reportes_P09\MFA"
        if (-not (Test-Path $carpeta)) {
            New-Item -Path $carpeta -ItemType Directory -Force | Out-Null
        }
        Add-Content -Path $LogMFA -Value $linea
    } catch { }
}

# --- OBTENER USUARIO ACTUAL ---
$usuarioRaw = $env:USERNAME
$usuario    = $usuarioRaw -replace ".*\\", "" -replace "@.*", ""

# --- BUSCAR CLAVE DEL USUARIO ---
$ClaveTOTP = $ClavesPorUsuario[$usuario]

if (-not $ClaveTOTP) {
    Write-MFALog "Usuario sin clave MFA - acceso directo" "SIN_CLAVE"
    exit 0
}
...
if (-not $accesoConcedido) {
    ...
    Write-MFALog "3 intentos fallidos. Bloqueando cuenta." "BLOQUEADO"
