...
function Configurar-Carpetas {
    Write-Host "`n[4/6] Creando carpetas y permisos..." -ForegroundColor Cyan
    ...
    foreach ($dep in @("Cuates", "NoCuates")) {
        ...
        $acl.SetAccessRule((New-Object System.Security.AccessControl.FileSystemAccessRule("$Dominio\$nombreGrupo","Modify","ContainerInherit,ObjectInherit","None","Allow")))
        Set-Acl $rutaDep $acl
    ...
    foreach ($u in $usuarios) {
        ...
        Write-Host "      Carpeta: $rutaPrivada" -ForegroundColor Green
    }
}

function Configurar-GPO-Logoff {
    Write-Host "`n[5/6] Configurando GPO de cierre forzado..." -ForegroundColor Cyan
    $dominioDN = (Get-ADDomain).DistinguishedName
...
function Configurar-AppLocker {
    ...
    $xmlBase = '<AppLockerPolicy Version="1"><RuleCollection Type="Exe" EnforcementMode="Enabled"><FilePathRule Id="921cc481-6e17-4653-8f75-050b80acca20" Name="Permitir Program Files" Description="" UserOrGroupSid="S-1-1-0" Action="Allow"><Conditions><FilePathCondition Path="%PROGRAMFILES%\*"/></Conditions></FilePathRule><FilePathRule Id="a61c8b2c-a319-4cd0-9690-d2177cad7e51" Name="Permitir Windows" Description="" UserOrGroupSid="S-1-1-0" Action="Allow"><Conditions><FilePathCondition Path="%WINDIR%\*"/></Conditions></FilePathRule><FilePathRule Id="fd686d83-a829-4351-8ff4-27c7de5755d2" Name="Permitir Admins" Description="" UserOrGroupSid="S-1-5-32-544" Action="Allow"><Conditions><FilePathCondition Path="*"/></Conditions></FilePathRule></RuleCollection></AppLockerPolicy>'
    ...
    foreach ($col in $polNotepad.RuleCollections) { foreach ($regla in $col) { $regla.Action = 'Deny' } }
...
function Mostrar-Menu {
    ...
    Write-Host "  [6]  Configurar FSRM"                     -ForegroundColor Cyan
...
do {
    ...
    switch ($opcion.ToUpper()) {
        ...
        "A" { Ejecutar-Todo }
        "G" { Write-Host "`nEjecutando gpupdate /force..." -ForegroundColor Cyan; gpupdate /force }
        "S" { Write-Host "`nSaliendo..." -ForegroundColor Red }
