# ============================================================
# menu_principal.ps1 — Punto de entrada único
...
. "$PSScriptRoot\menus.ps1"   # menus.ps1 ya carga functions.ps1
...
while ($true) {
    ...
    Write-Host ("║  Equipo : {0,-33}║" -f $env:COMPUTERNAME) -ForegroundColor Cyan
    $ipLocal = (Get-NetIPAddress -AddressFamily IPv4 |
        Where-Object { $_.IPAddress -ne "127.0.0.1" -and $_.PrefixOrigin -ne "WellKnown" } |
        Select-Object -First 1 -ExpandProperty IPAddress)
    Write-Host ("║  IP     : {0,-33}║" -f $ipLocal) -ForegroundColor Cyan
    ...
    switch ($opcion) {
        ...
        "2" { Menu-P2-DHCP-DNS }
        "3" { Menu-P3-SSH }
        "0" { Write-Host ""; Log-Aviso "Saliendo. Hasta luego!"; exit 0 }
